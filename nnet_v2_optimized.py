...
class Edge_Sheaf_NNet(nn.Module):
    ...
    def forward(self, xembed, ylabel, ylprob, wgraph, idvert):
        ...
        if graph_data['num_edges'] == 0:
            ...
            source_features = xembed[graph_data['source_indices']]
            target_features = xembed[graph_data['target_indices']]
        ...
        if graph_data['num_edges'] > 0:
            source_features = xmaped[graph_data['source_indices']]
            target_features = xmaped[graph_data['target_indices']]
            edge_matrices_batch = self.get_edge_matrix(source_features, target_features)
            messages = torch.bmm(edge_matrices_batch, source_features.unsqueeze(2)).squeeze(2)
        ...
        loss_cons = torch.tensor(0.0, device=xembed.device)
        if graph_data['num_edges'] > 0:
            final_source_features = xmaped[graph_data['source_indices']]
            final_target_features = xmaped[graph_data['target_indices']]
